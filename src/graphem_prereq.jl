...
function _kalman(y, A, H, Q, R, μ₀, Σ₀; drop_priors = true)
    K = size(y, 2)
    _state_dim = size(A, 1)
...
function _prox_f1(A, ϑ, Q, RTSS)
    ...
    #     temp = ϑ * K / Q[1]^2
    #     # @info("diag shortcut")
    #     return (temp * C + A) * pinv(Φ * temp + I(size(Q, 1)))
    # end
...
function _MS_l1_stab(A, Q, _RTSS; γ = 0.663, η = 0.99, r = 0.01, MS_iterations = 1000, ξ = 1e-4)
    # X = V1 = V2 = P1 = P1p = A
    X = copy(A)
    V1 = copy(A)
    V2 = copy(A)
    ...
    @inline objective(A1, A2) = majorant(A1) .- majorant(A2)
    for iter in 1:MS_iterations
        ...
        P1 = P1p
        P21 = Y21 .- γ .* _prox_f1(Y21 ./ γ, 1 / γ, Q, _RTSS)
        P22 = Y22 .- γ .* _prox_stable(Y22 ./ γ, η)
...
function GraphEM_stable(
    ...
    η = 0.99,
    r = 2.0,
    A₀ = _prox_stable(_create_adjacency_AR1(size(Q, 1), 0.1) .+ 4 .* randn(size(Q)), η),
    E_iterations = 50,
    M_iterations = 1000,
    ϵ = 1e-3,
    ξ = 1e-4,
    ...
    for s in 1:E_iterations
        ...
        # Ap = _MS_l1_stab(A, Q, RTSS_output; γ = γ, η = η, r = r, MS_iterations = M_iterations, ξ = ξ)
